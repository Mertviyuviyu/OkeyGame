...
public class Player {
    ...
    public int findLongestChainOf(Tile t) {
        ...
        int lastTile = tilePosition;
        for(int i = tilePosition;i>0; i--){
            if(playerTiles[lastTile].canFormChainWith(playerTiles[i])==1){
        ...
        for(int i = tilePosition+1;i<playerTiles.length && i >= 0;i++){
            if(playerTiles[lastTile] == null || playerTiles[i] == null){

            }
            else if(playerTiles[lastTile].canFormChainWith(playerTiles[i])==1){
                longestChainColorFirst++;
                lastTile = i;
            }
        }
        ...
        for(int i = tilePosition;i>0; i--){
            if(playerTiles[lastTile].canFormChainWith(playerTiles[i])==2){
        ...
        for(int i = tilePosition+1;i<playerTiles.length && i >= 0;i++){
            ...
            else if(playerTiles[lastTile].canFormChainWith(playerTiles[i])==2){
                longestChainValueFirst++;
                lastTile = i;
            }
        }
    ...
    public void sortTilesColorFirst() {
        for (int i = 0; i < playerTiles.length; i++) {
            for (int j = 1; j < playerTiles.length - i; j++) {
                if( playerTiles[j-1] == null){
                    Tile temp = playerTiles[14];
                    playerTiles[j-1] = temp;
                    playerTiles[14] = null;
                }
                else if( playerTiles[j] == null){
                    Tile temp = playerTiles[14];
                    playerTiles[j] = temp;
                    playerTiles[14] = null;
                }
                else if (playerTiles[j-1].compareToColorFirst(playerTiles[j]) == -1) {
        ...
        Collections.reverse(Arrays.asList(playerTiles));
    ...
    public void sortTilesValueFirst() {
        for (int i = 0; i < playerTiles.length; i++) {
            for (int j = 1; j < playerTiles.length - i; j++) {
                ...
                else if (playerTiles[j-1].compareToValueFirst(playerTiles[j ]) == -1) {
                    Tile temp = playerTiles[j];
                    playerTiles[j] = playerTiles[j - 1];
                    playerTiles[j - 1] = temp;
                }
    ...
    public int findPositionOfTile(Tile t) {
        ...
        for (int i = 0; i < numberOfTiles; i++) {
            ...
            else if (playerTiles[i].matchingTiles(t)) {
                tilePosition = i;
            }
        }
        return tilePosition;
